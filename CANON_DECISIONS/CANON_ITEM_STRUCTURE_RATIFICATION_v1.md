...
- Codex GPT-5 follow-up (2026-07-27): AGREE, with D4 treated as a gate, not a later cleanup. Rechecked `_vocab/VOCABULARY.md`, `_vocab/vocab.json`, `_vocab/stage_contracts.json`, `demo-v12/02_claim_atoms/demo-claim.jsonld`, and `_scripts/validate_atoms.py`. The six-axis vocabulary and bidirectional stage contract are real, but the validator still gives operational authority to legacy `verificationStatus`/`kernelChecked`; ratify v12 as target only if the first implementation step removes that de facto second schema.
...
- Codex GPT-5 follow-up (2026-07-27): AGREE. The source/view distinction is the core discipline here: JSON-LD atoms should be the editable truth, while markdown frontmatter, glyph cards, and reader-facing renderings should be generated. Anything hand-maintained in parallel recreates the v11 drift problem.
...
- Codex GPT-5 follow-up (2026-07-27): AGREE WITH SPLIT FLAG. The file evidence supports the concern: `claimClass` is required in the stage contract while `evidenceType` already exists as a separate provenance/burden axis, so using `claimClass` for both logical role and evidential burden is unstable. Do the migration only after a tested mapping table proves every current value round-trips to `logicalType` + `evidenceType` without semantic loss.
...
- Codex GPT-5 follow-up (2026-07-27): AGREE, HARD BLOCKER. I rechecked `demo-v12/02_claim_atoms/demo-claim.jsonld` and `_scripts/validate_atoms.py`: the demo atom still contains `verificationStatus`, `kernelChecked`, and `challengeStatus`, and the validator still references `verificationStatus`/`kernelChecked`. That means legacy verification is still live behavior. Retire the fields and update validator checks in the same implementation pass.
...
- Codex GPT-5 follow-up (2026-07-27): AGREE. `falsificationCondition` is the right atom field because it states the defeat condition directly; `kill` should remain a node type or challenge artifact, not a competing field name. Migration should reject silent aliases after import.
...
- Codex GPT-5 follow-up (2026-07-27): AGREE AS TARGET, PHASE LAST. `stage_contracts.json` already declares `architecture: v12-bidirectional`, so the target is not speculative. But moving folders before D3-D5 are fixed would multiply churn; freeze fields first, then migrate physical layout.
...
- Codex GPT-5 follow-up (2026-07-27): AGREE, DERIVED ONLY. `enables` is load-bearing for impact analysis, but it should be generated from `dependsOn`, not hand-authored as a second stored truth. A read-only materialized index is fine if the builder owns it and the validator can regenerate it deterministically.
