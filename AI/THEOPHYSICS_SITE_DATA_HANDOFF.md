...
## HTML To Markdown Correlation Prompt

If David asks whether the best HTML pages are correctly correlated with the cleaned Markdown corpus, use this prompt:

`\\192.168.2.50\h_hp\Desktop\ai-crew\PROJECTS\faiththruphysics-site\PROMPT_HTML_TO_MARKDOWN_CORRELATION_AUDIT.md`

Important: the primary "sheet" for that job is currently CSV, not Excel:
...
That coverage CSV should be worked alongside:

- `best-candidates\BEST_CANDIDATES.csv`
- `human-review\HUMAN_REVIEW_REQUIRED.csv`
- `identity-map\MASTER_HTML_IDENTITY_MAP_DEDUPED.csv`

The cleaned Markdown corpus to compare against is:

`E:\Faith Through Physics MD`

The full workflow is:

1. Use the coverage CSV to find article-like HTML identities still missing Markdown.
2. Choose the best HTML source using `BEST_CANDIDATES.csv` and `HUMAN_REVIEW_REQUIRED.csv`.
3. Send only the selected missing HTML pages through:

`\\192.168.2.50\brain\conversion_station`

4. Clean the converted Markdown with:

`C:\theophysics\_scripts\clean_markdown.py`

5. Import cleaned Markdown into the existing short-code folders under:

`E:\Faith Through Physics MD`

6. Rerun/update the coverage report so the missing count drops from the current known article-like gap of 420.

Do HTML selection first. Do not convert from a stub, backup, template, or worse page when a better HTML version exists elsewhere.
